...
(defn render-parsed-text-with-timestamp [{:keys [timestamp-str outgoing in-popover?] :as message} tree]
  (let [elements (render-parsed-text message tree)
        timestamp [react/text {:style (style/message-timestamp-placeholder)}
                   (str (if (and outgoing (not in-popover?)) "        " "  ") (when-not in-popover? timestamp-str))]
...
(defn pin-limit-popover []
  (let [{:keys [message]} (<sub [:popover/popover])]
    [react/view
     ...
     [pinned-messages-view (message :chat-id)]
